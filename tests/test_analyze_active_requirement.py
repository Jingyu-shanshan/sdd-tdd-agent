...
def test_should_analyze_active_session_through_configured_adapter(
    tmp_path: Path,
) -> None:
...
""",
    ...
    runner = AnalysisResultRunner()

    run = analyze_active_requirement(tmp_path, runner)

    assert runner.command == ("bridge", "analyze")
